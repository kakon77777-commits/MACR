...
- The checksum carried by the task is integrity evidence, not authority. Authority is a separate D: host-approval record with fixed `host_operator`, UUID nonce, creation time, and expiry. `glm-approve` is an explicit host action; absent, malformed, expired, or tampered records fail closed.
...
- `glm-preflight` validates policy, exact request digest, and external host record without reading a credential and emits only content-free metadata. The wrapper never reads the key. Provider invocation reparses the current task, repeats all approval checks, then reads only fixed `D:\KEY\GLM.txt`; this closes the preflight/invoke TOCTOU window before credential resolution.
