...
- a digest-bound deployment mode separating the canonical operator runtime
  from explicit offline-test kernels;
...
access. Production transport additionally requires the canonical runtime path
selected by the operator state root. A temp-path kernel is permanently marked
`offline_test`; it can be used only with an explicitly injected non-production
transport and cannot be reopened as the canonical deployment.
...
Immediately before transport, the kernel reacquires the SQLite write lock,
applies expiry, validates the latest control receipt, checks unresolved/circuit
state, and revalidates dispatch authority inside that same transaction. An
expired grant becomes reconciliation; a still-unused grant superseded by a
known open circuit is cancelled and cannot reach transport.
...
schema-7 fixture generated by `main@f806fdb`. Provider-admission bootstrap also
requires external quiescence: any legacy nonterminal run or active dispatch
lease blocks genesis rather than being interpreted as free capacity. Normal
rollback is a governed
