...
- immutable provider admission policy rows and one effective state row;
...
Explicit injected test guards remain test-only dependencies.
...
Half-open is a separately authorized single-flight probe, not elapsed-time
recovery. No fixed cooldown is claimed from current evidence.
...
Schema migration is additive and copy-tested. Normal rollback is a governed
