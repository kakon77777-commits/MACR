...
Token-policy state is append-only in store schema 1 at `settings\model-token-policies.sqlite3`. Policy contract v2 serializes its minimum task output into the digest. New Direct conversations pin one exact provider/model policy; old conversations are not rewritten, and legacy Grok snapshots cannot dispatch. Delegated runtime records the exact policy digest, and GLM approval schema 3 binds the same digest plus context/output envelope. The ordinary presets deliberately differ: Grok hard context 400,000 and output floor/default 32,768; GLM/Gemini 512,000 with floor/default 16,384; MiniMax 180,000 with immutable provider-limited output 2,048; Qwythos-9B-v2 8,192 / 4,096 without a cloud floor. The T1 GLM preset is a separate 128,000 / 16,384.

T1 schema-3 staging requires a strict three-member manifest, three existing GLM host approvals, exact T1 token-policy-v2 digest, batch authority, dispatch authority, 16,384 output tokens, USD 0.010 per member, USD 0.030 aggregate and USD 0.040 campaign. Schema 2 is immutable audit evidence reported as `legacy_pre_quality_floor` and cannot satisfy the new route. One worker claims at most one plan-scoped member. Provider completion does not verify, materialize, accept, merge, or deploy. Dispatch ambiguity enters global `reconciliation_required`, blocks later claims, and explicit resolution revokes the old batch so only a separately authorized new manifest can continue.
