...
The final delegated-worker gate requires `non_sensitive_routine` plus a SHA-256 approval manifest over the complete credential-free request payload, exact provider/endpoint/model, task type, privacy, maximum output, conservative USD ceiling, and pricing basis. Integrity alone is not authority: a separate content-free D: host record binds the digest to a fixed author role, nonce, creation time, and expiry. The wrapper never reads the credential; provider invocation repeats approval against the current task before reading fixed `D:\KEY\GLM.txt`. Semantic classification remains a trusted operator decision and is not inferred from model content.
